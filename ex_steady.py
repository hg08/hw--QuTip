...
a = qt.destroy(N)
H = a.dag() * a
psi0 = qt.basis(N, 10)  # initial state
...
final_state = qt.steadystate(H, c_op_list)
# find expectation value for particle number in steady state
fexpt = qt.expect(a.dag() * a, final_state)

tlist = np.linspace(0, 50, 100)
# monte-carlo
mcdata = qt.mcsolve(H, psi0, tlist, c_op_list, [a.dag() * a], ntraj=100)
# master eq.
medata = qt.mesolve(H, psi0, tlist, c_op_list, [a.dag() * a])
